...
| 변수 | 값 | 비고 |
|---|---|---|
| `CP_2B_INSTANCE_ID` | Track B 배포 후 확인 | `prod-ec2-k8s-cp-2b` |
...
```bash
# ── 인스턴스 ID ──
export CP_2B_INSTANCE_ID="<cp-2b 인스턴스 ID — Track B 배포 후 확인>"
...
export LOG_DIR="/tmp/fi-04-${EXPERIMENT_TS}"
mkdir -p "${LOG_DIR}"
```
...
- 실행 위치: `prod-ec2-k8s-cp-2a` (SSM Session Manager)
- 목적: 실험 전 etcd 상태 보존 (최악의 경우 복구 기준점)
- 명령어:

```bash
sudo ETCDCTL_API=3 etcdctl snapshot save \
  /tmp/etcd-fi04-pre-$(date +%Y%m%d-%H%M%S).db \
...
# 스냅샷 유효성 확인
sudo ETCDCTL_API=3 etcdctl snapshot status /tmp/etcd-fi04-pre-*.db \
  --write-out=table
```

- 기대 결과:
  - snapshot hash, revision, total keys 정상 출력
- 실패 징후:
...
- 롤백 / 정리:
  - `aws iam delete-role-policy --role-name prod-fis-experiment-role --policy-name fis-ec2-stop-policy`
  - `aws iam delete-role --role-name prod-fis-experiment-role`
...
aws cloudwatch put-metric-alarm \
  --alarm-name "fi04-stop-apiserver-tg-unhealthy" \
...
- 롤백 / 정리:
  - `aws cloudwatch delete-alarms --alarm-names fi04-stop-apiserver-tg-unhealthy`
...
```bash
cat > /tmp/fi04-template.json << 'TEMPLATE'
{
...
sed -i '' \
  ...
  /tmp/fi04-template.json

# 템플릿 등록
aws fis create-experiment-template \
  --cli-input-json file:///tmp/fi04-template.json \
  --region ${AWS_REGION} \
...
- 기대 결과:
  - Template ID 반환
- 실패 징후:
  - `ValidationException` → JSON 구조 오류 → `cat /tmp/fi04-template.json | jq .` 로 포맷 확인
...
- 실행 위치: `로컬 작업 PC` (**별도 터미널**)
- 목적: 실험 전~중~후 서비스 가용성을 1초 단위로 기록 (부하 없이 단순 health check)
- 명령어:

```bash
# ── 별도 터미널에서 실행 ──
# Step A-1 터미널에서 echo $LOG_DIR 로 확인한 경로를 붙여넣기
# 또는 자동 탐색:
export LOG_DIR=$(ls -dt /tmp/fi-04-* 2>/dev/null | head -1)
...
- 실패 징후:
  ...
  - `status: failed` → FIS 로그 확인: `aws logs tail /aws/fis/fi04`
...
- 기대 결과:
  ...
  - EC2 상태: `stopped`
- 실패 징후:
  - 5분 이상 `Ready` 유지 → FIS action이 실행되지 않음 → `aws fis get-experiment` 로 상태 재확인
...
- 실행 위치: `prod-ec2-k8s-cp-2a` (SSM) + `로컬 작업 PC`
- 목적: cp-2b 손실 후 etcd 2/3 quorum 유지, kubectl 정상 응답 검증
- 명령어:

```bash
# [cp-2a SSM] etcd member 상태
sudo ETCDCTL_API=3 etcdctl member list \
  --endpoints=https://127.0.0.1:2379 \
  --cacert=/etc/kubernetes/pki/etcd/ca.crt \
  --cert=/etc/kubernetes/pki/etcd/server.crt \
...
# [cp-2a SSM] etcd endpoint health
...
- 실패 징후:
  - `status: running` → 아직 실행 중 → 완료 대기 또는 수동 중단: `aws fis stop-experiment --id ${FIS_EXPERIMENT_ID}`
...
- 실패 징후:
  - `health-loop.csv` 없음 → C-3의 별도 터미널 `LOG_DIR` 경로 불일치 확인
...
aws fis delete-experiment-template --id ${FIS_TEMPLATE_ID}
aws cloudwatch delete-alarms --alarm-names fi04-stop-apiserver-tg-unhealthy
aws logs delete-log-group --log-group-name /aws/fis/fi04
