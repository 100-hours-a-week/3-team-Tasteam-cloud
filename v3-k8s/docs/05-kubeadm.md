...
v3에서는 NLB + NGINX Ingress Controller로 전환하면서 Caddy를 제거합니다.
정적 파일은 S3 + CloudFront(CDN)로 분리하고, API 트래픽은 NLB(L4) → NGINX Ingress(L7) → Pod 경로로 라우팅합니다.
...
| 서비스 라우팅 | Caddy + Cloud Map + Lambda SD 체인 | NLB + NGINX Ingress + K8s Service DNS |
...
현재 Caddy가 담당하는 외부 진입점(443)을 **NLB + NGINX Ingress Controller**로 전환합니다.

```
[클라이언트]
    │
    ▼
[Cloudflare] ── DNS + CDN + TLS 종단 (정적 파일은 S3 + CloudFront)
    │
    ▼
[NLB] ── L4 TCP 전달, 헬스체크, 정상 노드만 선택
    │
    ▼
[NGINX Ingress Controller] ── L7 경로 기반 라우팅
    │
...
Cloudflare → NLB → NGINX Ingress 구조에서 WebSocket 연결이 끊어지는 원인은 대부분 **타임아웃 설정의 부조화**입니다.
...
> **운영 주의**: Cloudflare → NLB → NGINX 체인에서 **가장 짧은 타임아웃이 실제 한계**입니다.
> NLB는 L4이므로 HTTP 타임아웃이 없고 TCP idle timeout(350초)만 존재합니다.
> WebSocket ping/pong heartbeat 주기를 Cloudflare 타임아웃(100초)과 NLB TCP idle timeout(350초)보다 짧게 설정해야 유휴 종료를 방지할 수 있습니다.
...
| Namespace | 용도 | 포함 리소스 |
|-----------|------|------------|
| `app-dev` | 개발·기능 테스트 | Spring Boot, FastAPI, Service, Ingress, ConfigMap, Secret |
| `app-stg` | 통합 테스트·부하 테스트 | Spring Boot, FastAPI, Service, Ingress, ConfigMap, Secret |
| `app-prod` | 운영 | Spring Boot, FastAPI, Service, Ingress, ConfigMap, Secret |
...
API 요청:   User → Cloudflare → NLB → NGINX Ingress → spring-boot-svc → Spring Pods
AI 요청:    User → Cloudflare → NLB → NGINX Ingress → fastapi-svc → FastAPI Pods
WebSocket:  User → Cloudflare → NLB → NGINX Ingress (/ws, timeout 3600s) → Spring Pods
...
AZ 2b 전체 장애
  ...
  → 내부 NLB가 정상 API Server로 라우팅, 외부 NLB가 정상 워커로 트래픽 분산
...
18:00 저녁 피크 — 채팅 동시 접속 급증
  ...
  → NLB TCP idle timeout(350초), NGINX proxy-read-timeout(3600초) 모두 heartbeat 주기보다 길게 설정
